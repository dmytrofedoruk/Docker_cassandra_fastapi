...
class Data:
    ...
    def get_resto(cls, id):
        cls.open_db()
        data = cls.session.execute(f"SELECT * FROM restaurant WHERE id={id}").one()
        cls.close_db()
        return data
    ...
    @classmethod
    def get_resto_inspec(cls, id):
        cls.open_db()
        resto = cls.session.execute(f"SELECT name FROM restaurant WHERE id = {id}").one()
        number = len(cls.session.execute(f"SELECT * FROM inspection WHERE idrestaurant = {id}").all())
        cls.close_db()
    ...
    def get_top10(cls, grade):
        ...
        filters = tuple(id['idrestaurant'] for id in top10)
